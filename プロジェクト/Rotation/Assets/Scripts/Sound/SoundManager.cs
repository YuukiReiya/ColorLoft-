...
public class SoundManager : SingletonMonoBehaviour<SoundManager> {
    ...
    /// </summary>
    /// <param name="key">登録されたサウンドリストのハッシュキー</param>
    /// <param name="channelIndex">流すチャネルの番号</param>
    /// <param name="volume">音量(0.0～1.0)</param>
    public void PlayOnSE(string key, int channelIndex, float volume = 1.0f)
    {
        //ハッシュのチェック
        if (!CheckSoundsKey(key))
        {
        ...
        //再生処理
        AudioClip clip = sounds.Table[key];
        channel[channelIndex].PlayOneShot(clip, volume);
    }
    ...
    /// <param name="channelIndex">流すチャネルの番号</param>
    /// <param name="volume">音量(0.0～1.0)</param>
    public void PlayOnSE(int soundIndex, int channelIndex, float volume = 1.0f)
    {
        //ハッシュのチェック
        if (!CheckSoundsKey(soundIndex))
        {
